...
// ---------------------------------------------------------------------//
// check if PFString is equal to selection
// ---------------------------------------------------------------------//
    function checkPFString(PFString){
        var retCode = 'false';
        var splitPFString = PFString.split(',');
        splitPFString.push('-All-'); // add -All- to every PFString so if selection is -All- it allways returns 'valid'
        // check if Array selectedTestPlatform has equal entries to PF String Array
        for (var i in splitPFString){
            if(selectedTestPlatform.indexOf(splitPFString[i].trim()) > -1){
                retCode = 'valid';
            }
        }
        return retCode;
...
//                    console.log('-------------------------------------------------------------------');
            ...
            case 'scr':
                ...
                if(Type == 'SOW Requirement' && validTPF == 'valid' && useSILReq == 'true') {
                    activeReq = 'true';
                    scr_id = this.getElementsByClassName('ID')[1].childNodes[0].nodeValue;
                    ReqMilestone = Milestone;
                    SOWReqCount++;
//                    console.log('SOWID:', scr_id, 'SOW State:', State, 'Planned for:', Milestone);
//                    console.log('-------------------------------------------------------------------');
                }
